...
std::list<int>* getAvailablePorts() {
	...
	LPWSTR lpTargetPath = (LPWSTR)calloc(5000, sizeof(char));
	std::list<int>* portList = new std::list<int>();
...
void createPortFile(HANDLE* hCom, wchar_t* COMPORT) {
	...
	*hCom = CreateFile(
	 	utf16ToUTF8(COMPORT).c_str(),									// COM port number  --> If COM# is larger than 9 then use the following syntax--> "\\\\.\\COM10"
